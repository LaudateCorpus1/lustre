...
#define IBLND_CREDIT_HIGHWATER_V1                                              \
  7 /* V1 only : when eagerly to return credits */

#define IBLND_CREDITS_DEFAULT 8 /* default # of peer_ni credits */
#define IBLND_CREDITS_MAX                                                      \
  ((typeof(((struct kib_msg *)0)->ibm_credits)) -                              \
   1) /* Max # of peer_ni credits */
...
#else
#define kiblnd_rdma_create_id(ns, cb, dev, ps, qpt) rdma_create_id(cb, dev, ps)
#endif
#endif
...
struct kib_hca_dev {
  ...
  struct ib_event_handler ibh_event_handler; /* IB event handler */
...
typedef int (*kib_ps_pool_create_t)(struct kib_poolset *ps, int inc,
...
#define IBLND_POSTRX_RSRVD_CREDIT                                              \
  3 /* post: give myself back 1 reserved credit */
...
static inline int kiblnd_concurrent_sends(int version, struct lnet_ni *ni) {
...
static inline void kiblnd_hdev_addref_locked(struct kib_hca_dev *hdev) {
  LASSERT(atomic_read(&hdev->ibh_ref) > 0);
  atomic_inc(&hdev->ibh_ref);
}

static inline void kiblnd_hdev_decref(struct kib_hca_dev *hdev) {
  LASSERT(atomic_read(&hdev->ibh_ref) > 0);
  if (atomic_dec_and_test(&hdev->ibh_ref))
    kiblnd_hdev_destroy(hdev);
}

static inline int kiblnd_dev_can_failover(struct kib_dev *dev) {
...
#define kiblnd_conn_decref(conn)                                               \
  do {                                                                         \
    ...
    if (atomic_dec_and_test(&(conn)->ibc_refcount)) {                          \
      ...
      list_add_tail(&(conn)->ibc_list, &kiblnd_data.kib_connd_zombies);        \
...
static inline bool kiblnd_peer_connecting(struct kib_peer_ni *peer_ni) {
  return peer_ni->ibp_connecting != 0 || peer_ni->ibp_reconnecting != 0 ||
         peer_ni->ibp_accepting != 0;
}

static inline bool kiblnd_peer_idle(struct kib_peer_ni *peer_ni) {
  return !kiblnd_peer_connecting(peer_ni) && list_empty(&peer_ni->ibp_conns);
}

static inline struct list_head *kiblnd_nid2peerlist(lnet_nid_t nid) {
  unsigned int hash = ((unsigned int)nid) % kiblnd_data.kib_peer_hash_size;

  return &kiblnd_data.kib_peers[hash];
}

static inline int kiblnd_peer_active(struct kib_peer_ni *peer_ni) {
...
static inline int kiblnd_send_keepalive(struct kib_conn *conn) {
...
static inline int kiblnd_need_noop(struct kib_conn *conn) {
  ...
  if (conn->ibc_outstanding_credits < IBLND_CREDITS_HIGHWATER(tunables, conn) &&
...
static inline void kiblnd_abort_receives(struct kib_conn *conn) {
  ib_modify_qp(conn->ibc_cmid->qp, &kiblnd_data.kib_error_qpa, IB_QP_STATE);
}

static inline const char *kiblnd_queue2str(struct kib_conn *conn,
                                           struct list_head *q) {
...
static inline __u64 kiblnd_ptr2wreqid(void *ptr, int type) {
...
static inline void *kiblnd_wreqid2ptr(__u64 wreqid) {
  return (void *)(((unsigned long)wreqid) & ~IBLND_WID_MASK);
}

static inline int kiblnd_wreqid2type(__u64 wreqid) {
  return (wreqid & IBLND_WID_MASK);
}

static inline void kiblnd_set_conn_state(struct kib_conn *conn, int state) {
  conn->ibc_state = state;
  smp_mb();
}

static inline void kiblnd_init_msg(struct kib_msg *msg, int type,
                                   int body_nob) {
  msg->ibm_type = type;
  msg->ibm_nob = offsetof(struct kib_msg, ibm_u) + body_nob;
}

static inline int kiblnd_rd_size(struct kib_rdma_desc *rd) {
  int i;
  int size;
...
static inline __u64 kiblnd_rd_frag_addr(struct kib_rdma_desc *rd, int index) {
  return rd->rd_frags[index].rf_addr;
}

static inline __u32 kiblnd_rd_frag_size(struct kib_rdma_desc *rd, int index) {
  return rd->rd_frags[index].rf_nob;
}

static inline __u32 kiblnd_rd_frag_key(struct kib_rdma_desc *rd, int index) {
  return rd->rd_key;
}

static inline int kiblnd_rd_consume_frag(struct kib_rdma_desc *rd, int index,
                                         __u32 nob) {
...
static inline int kiblnd_rd_msg_size(struct kib_rdma_desc *rd, int msgtype,
                                     int n) {
  LASSERT(msgtype == IBLND_MSG_GET_REQ || msgtype == IBLND_MSG_PUT_ACK);
...
static inline __u64 kiblnd_dma_mapping_error(struct ib_device *dev,
                                             u64 dma_addr) {
  return ib_dma_mapping_error(dev, dma_addr);
}

static inline __u64 kiblnd_dma_map_single(struct ib_device *dev, void *msg,
                                          size_t size,
                                          enum dma_data_direction direction) {
  return ib_dma_map_single(dev, msg, size, direction);
}

static inline void kiblnd_dma_unmap_single(struct ib_device *dev, __u64 addr,
                                           size_t size,
                                           enum dma_data_direction direction) {
  ib_dma_unmap_single(dev, addr, size, direction);
}

#define KIBLND_UNMAP_ADDR_SET(p, m, a)                                         \
  do {                                                                         \
  } while (0)
...
int kiblnd_close_stale_conns_locked(struct kib_peer_ni *peer_ni, int version,
                                    u64 incarnation);
...
struct kib_conn *kiblnd_create_conn(struct kib_peer_ni *peer_ni,
                                    struct rdma_cm_id *cmid, int state,
                                    int version);
